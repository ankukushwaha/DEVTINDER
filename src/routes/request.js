  ...
  async (req, res) => {
    try {
      ...
      if (!isValidStatus) {
        return res
          .status(400)
          .send("Invalid status! Use 'interested' or 'ignored'.");
      }
      ...
      const senderUserName = req.user.firstName;

      const isValidRequest = await connectionModel.findOne({
        $or: [
      ...
      // sending email
      const subject = `${senderUserName} sent you a friend request`;
      const html = `
      <h2>Hello ${isReceiverInDB.firstName},</h2>
      <p>You have a new friend request from <b>${senderUserName}</b>.</p>
    ...
    `;
      ...
      return res
        ...
        .json(
          {
            message: `${req.user.firstName} is ${status} to ${isReceiverInDB.firstName} `,
          },
          data
        );
    } catch (err) {
  ...
  async (req, res) => {
    try {
      ...
      if (!isValidStatus) {
        return res
          .status(400)
          .send("Invalid status! Use 'accepted' or 'rejected' only.");
      }
      ...
      if (!connectionRequest) {
        return res
          .status(404)
          .send("You don't have any pending connection requests!");
      }
